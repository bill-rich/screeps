...
module.exports.loop = function () {
    ...
    if(tower) {
        ...
        for(i=0; i<structs.length; i++){
            if(structs[i].structureType != STRUCTURE_WALL || structs[i].structureType == STRUCTURE_WALL && structs[i].hits < 10){
    ...
    if(spawnEnergy >= 400) {
        ...
        if(miners.length < 1) {
            ...
            Game.spawns['TheFort'].spawnCreep([MOVE,WORK,WORK,WORK], newName,
                {memory: {role: 'miner'}});
        }
    }
    var i=0
